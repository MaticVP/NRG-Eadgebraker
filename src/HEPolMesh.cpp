...
vector<HEmesh> HEmesh::findDisconnectedMeshes() {
    int polyCount = polylist.size();
    vector<bool> visited(polyCount, false);
    vector<HEmesh> meshes;

    numberOfDisconnectedMesh = 0;
    startCorners.clear();
    startPolygon.clear();

    for (int i = 0; i < polyCount; i++) {
        if (!visited[i]) {
            numberOfDisconnectedMesh++;

            std::cout << "Component " << numberOfDisconnectedMesh << std::endl;

            std::vector<int> polyStack;
            std::set<int> usedPolygons;
            std::set<int> usedVertices;

            polyStack.push_back(i);

            while (!polyStack.empty()) {
                int current = polyStack.back();
                polyStack.pop_back();

                if (visited[current]) continue;
                visited[current] = true;

                usedPolygons.insert(current);

                int startHE = polylist[current].firstHE;
                int he = startHE;

                do {
                    usedVertices.insert(edgelist[he].v);

                    int inv = edgelist[he].invHE;
                    if (inv != -1) {
                        int neighPol = edgelist[inv].polygon;
                        if (neighPol != -1 && !visited[neighPol]) {
                            polyStack.push_back(neighPol);
                        }
                    }
                    he = edgelist[he].nextHE;
                } while (he != startHE);
            }

            // Construct a new mesh for this component
            HEmesh newMesh;
            std::map<int, int> oldToNewIndex;

            // Add vertices
            for (int oldIdx : usedVertices) {
                int newIdx = newMesh.vertexlist.size();
                newMesh.addVertex(vertexlist[oldIdx].pt);
                oldToNewIndex[oldIdx] = newIdx;
            }

            // Add polygons
            for (int polyIdx : usedPolygons) {
                int startHE = polylist[polyIdx].firstHE;
                int he = startHE;
                vector<int> polyVertices;

                do {
                    polyVertices.push_back(oldToNewIndex[edgelist[he].v]);
                    he = edgelist[he].nextHE;
                } while (he != startHE);

                int poly[3] = { polyVertices[0], polyVertices[1], polyVertices[2] };
                newMesh.addPolygon(3, poly);
            }
    ...
    std::cout << "Disconnected components found: " << numberOfDisconnectedMesh << std::endl;
    return meshes;
}
